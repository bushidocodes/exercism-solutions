...
static inline bool can_attack_diagonally(position_t queen_1,
                                         position_t queen_2) {
	// Diagonal can be determined by the rows and columns have the same unsigned
	// difference
	int diff_cols = queen_2.column - queen_1.column;
	if (diff_cols < 0) diff_cols *= -1;
