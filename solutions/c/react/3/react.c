...
struct reactor *create_reactor() {
	...
	struct reactor *result =
	    malloc(sizeof(struct reactor) + sizeof(struct cell) * capacity);
...
void destroy_reactor(struct reactor *r) { free(r); }

struct cell *create_input_cell(struct reactor *r, int initial_value) {
	if (r->cells_count == r->cells_capacity) {
		r->cells_capacity *= 2;
		r = realloc(r, sizeof(struct reactor) +
		                   sizeof(struct cell) * r->cells_capacity);
	}
	...
	switch (c->state) {
		...
		case COMPLETE: {
			...
			for (int i = 0; i < c->callbacks_count; i++) {
				...
				if (cb != NULL && args != NULL) cb(args, new_value);
			}
...
static void fire_trigger(struct trigger *t) {
	switch (t->variant) {
		...
		case COMPUTE_2: {
			// If any of the input cells are PENDING, NOOP to avoid excessive
			// invocations
			if (t->input_a->state == UNINITALIZED ||
			    t->input_b->state == UNINITALIZED ||
			    t->input_a->state == PENDING || t->input_b->state == PENDING)
				return;

			int new_value = t->compute_2(get_cell_value(t->input_a),
			                             get_cell_value(t->input_b));

			switch (t->output->state) {
				case UNINITALIZED: {  // If the output cell hasn't yet been
					                  // initialized, set value
					set_cell_value(t->output, new_value);
					return;
				}
				case PENDING: {  // If an update, only actually update if there
					             // is a change
					int original_value = get_cell_value(t->output);
					if (new_value != original_value)
						set_cell_value(t->output, new_value);
...
struct cell *create_compute1_cell(struct reactor *r, struct cell *input_cell,
                                  compute1_t f) {
	// Add cell in pending state
	struct cell *output_cell = &r->cells[r->cells_count++];
	*output_cell = (struct cell){.state = UNINITALIZED, .triggers_count = 0};

	// Add Trigger
	struct trigger *t = &r->triggers[r->triggers_count++];
	*t = (struct trigger){.variant = COMPUTE_1,
	                      .input_a = input_cell,
	                      .input_b = NULL,
	                      .output = output_cell,
	                      .compute_1 = f,
...
struct cell *create_compute2_cell(struct reactor *r, struct cell *input_cell_a,
                                  struct cell *input_cell_b, compute2_t f) {
	assert(r != NULL);
	...
	*output_cell = (struct cell){.state = UNINITALIZED, .triggers_count = 0};
	...
	// Add Trigger
	struct trigger *t = &r->triggers[r->triggers_count++];
	*t = (struct trigger){.variant = COMPUTE_2,
