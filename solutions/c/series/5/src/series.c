...
series_results_t series(char *input_text, unsigned int substring_length)
{
    ...
    bool is_valid = input_length <= MAX_INPUT_TEXT_LENGTH &&
                    substring_length > 0 &&
                    substring_length <= input_length &&
                    substring_length <= MAX_SERIES_LENGTH;

    series_results_t result = {
        .substring_count = is_valid ? input_length - (substring_length - 1) : 0,
        .substring = is_valid ? calloc(sizeof(char *), result.substring_count) : &input_text};
