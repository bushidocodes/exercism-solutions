...
product_t *get_palindrome_product(int from, int to)
{
    ...
    if (from > to)
    {
        snprintf((char *restrict)(&results->error), MAXERR, "invalid input: min is %d and max is %d", from, to);
        return results;
    }
    ...
    if (results->largest == INT_MIN)
        snprintf((char *restrict)(&results->error), MAXERR, "no palindrome with factors in the range %d to %d", from, to);
