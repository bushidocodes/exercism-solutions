...
char *decode(char *ciphertext, size_t rails) {
	...
	for (size_t i = 0; i < ciphertext_len;) {
		for (size_t current_rail = 0;
		     current_rail < rails && i < ciphertext_len; current_rail++, i++) {
			rail_len[current_rail]++;
		}
		for (size_t current_rail = rails - 2;
		     current_rail > 0 && i < ciphertext_len; current_rail--, i++) {
			rail_len[current_rail]++;
		}
	}
	...
	while (i < ciphertext_len) {
		for (size_t current_rail = 0;
		     current_rail < rails && i < ciphertext_len; current_rail++, i++) {
			size_t src_idx =
			    rail_start[current_rail] + rail_idx[current_rail]++;
