...
static int get_digit(char **cursor)
{
    ...
    while (isdigit(**cursor) || isspace(**cursor))
        (*cursor)++;
...
get_operation(char **cursor)
{
    ...
    else if (strncmp(*cursor, CALC_MULTIPLY_TOK, CALC_MULTIPLY_TOK_LEN) == 0)
    {
...
bool answer(const char *question, int *result)
{
    ...
    if (strncmp(question, "What is ", strlen("What is ")) != 0)
        return false;
