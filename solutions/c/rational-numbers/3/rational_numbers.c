...
rational_t add(rational_t r1, rational_t r2) {
	...
	rational_t result =
	    (rational_t){.numerator = r1.numerator * r2.denominator +
...
rational_t multiply(rational_t r1, rational_t r2) {
	...
	rational_t result =
	    (rational_t){.numerator = r1.numerator * r2.numerator,
	                 .denominator = r1.denominator * r2.denominator};
...
rational_t absolute(rational_t r1) {
	...
	rational_t result = (rational_t){.numerator = abs(r1.numerator),
...
rational_t exp_rational(rational_t r1, uint16_t n) {
	...
	rational_t result = (rational_t){.numerator = pow(r1.numerator, n),
...
rational_t reduce(const rational_t r1) {
	assert(r1.denominator != 0);
	rational_t result =
	    (rational_t){.numerator = r1.numerator, .denominator = r1.denominator};
	...
	// What is smaller?
	bool numerator_is_smallest =
	    abs(result.numerator) <= abs(result.denominator);
	int smallest =
	    numerator_is_smallest ? result.numerator : result.denominator;
	int largest =
	    !numerator_is_smallest ? result.numerator : result.denominator;

	// Are they equal?
	if (smallest == largest)
		result = (rational_t){.numerator = 1, .denominator = 1};
	...
	// Actually factor stuff out
		...
		// Get the positive factors of the smaller (absolute) thing
		// between 2..half
		...
		for (int i = 2; i <= abs(smallest) / 2; i++) {
			if (buffer % i == 0) {
				...
				if (factors_size + 1 == FACTORS_CAPACITY) exit(EXIT_FAILURE);
