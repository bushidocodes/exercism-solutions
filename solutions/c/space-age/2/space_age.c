...
float age(planet_t planet, int64_t input) {
	if (planet >= PLANET_COUNT) return (float)-1;

	float age_in_earth_years = input / SECS_PER_MIN / MINS_PER_HOUR /
	                           EARTH_HOURS_PER_DAY / EARTH_DAYS_PER_YEAR;
