...
void find_anagrams(const char *subject, struct candidates *candidates) {
	...
	for (size_t i = 0; i < candidates->count; i++) {
		...
		for (size_t j = 0; j < strlen(candidate->candidate) && j < MAX_STR_LEN;
		     j++)
		...
		if (candidate->is_anagram) {
			...
			for (size_t j = 0;
			     j < strlen(candidate->candidate) && j < MAX_STR_LEN; j++)
				if (toupper(subject[j]) != toupper(candidate->candidate[j]))
