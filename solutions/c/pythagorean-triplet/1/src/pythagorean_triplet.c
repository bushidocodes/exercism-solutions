...
triplets_t *triplets_with_sum(uint16_t sum)
{
    ...
    for (int c = largest_possible_c; c >= smallest_feasible_c; c--)
    {
        ...
        for (int b = sqrt(c * c - smallest_feasible_a * smallest_feasible_a); b < c && b >= smallest_feasible_b; b--)
        {
            if (b != (float)(int)b)
                continue;
            ...
            {
                results->count++;
                results->triplets = realloc(results->triplets, sizeof(triplet_t) * results->count);
                results->triplets[results->count - 1] = (triplet_t){
                    .a = a,
                    .b = b,
                    .c = c};
            }
