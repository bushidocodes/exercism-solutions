...
size_t find_factors(uint64_t n, uint64_t factors[static MAXFACTORS]) {
	...
	for (uint32_t j = 0; j < vector_get_count(&primes); j++) {
		...
		if (!result.did_succeed) exit(EXIT_FAILURE);
	...
	// Get the last prime. We use the as the denominator to reduce our search
	// space
	uint32_t last_prime;
	result_t result = vector_get_last(&primes);
	if (result.did_succeed) last_prime = result.data;

	for (uint32_t candidate = last_prime + 2;
	     candidate <= buffer / (last_prime) && primes_found < MAXFACTORS;
	     candidate += 2) {
		bool is_prime = true;
	...
	if (buffer != 1) factors[primes_found++] = buffer;
