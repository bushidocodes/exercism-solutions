...
namespace meetup
{
    ...
    scheduler::scheduler(months_of_year month, year_t year) : month_{month}, year_{year} {};

    date_t scheduler::range(int start, int end_inclusive, boost::date_time::weekdays day) const
    {
        if (end_inclusive - start != 6)
        {
            throw new std::invalid_argument{"Start and End date (inclusive) must form a consecutive range of 7 days"};
        }

        /* This feels like unnecessary copies, but I'm unsure how to change a boost date's day after declaration */
        date_t result;
        for (int nth_day = start; nth_day <= end_inclusive; nth_day++)
        {
    ...
    {
        /* This feels wasteful to instantiate a class to get the length of the month, but Boost dates are immutable */
