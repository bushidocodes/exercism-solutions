...
cipher::cipher(const std::string &plaintext) { this->_plaintext = plaintext; }
...
std::vector<std::string> cipher::plain_text_segments() {
	...
	int c = 0;
	while (c * c < buffer_length) c++;

	int r = c - 1;
	if (c * r < buffer_length) r++;

	std::vector<std::string> text_segments;

	for (int row = 0; row < buffer_length; row += c) {
		std::string row_buffer = "";
		for (int column = 0; column < c && row + column < buffer_length;
		     column++) {
...
std::string cipher::cipher_text(bool normalize) {
	...
	if (text_segments.size() > 0) string_length = text_segments[0].length();

	std::string ciphertext = "";

	for (size_t i = 0; i < string_length; i++) {
		/* Delimit segments if normalize true */
		if (normalize && i > 0) ciphertext.push_back(' ');

		for (auto str : text_segments) {
			if (i <= str.length()) ciphertext.push_back(str[i]);
		}
	}
	return ciphertext;
}

std::string cipher::normalized_cipher_text() { return cipher_text(true); }
