       ...
       01 WS-CANDIDATES-TABLE.
          05 WS-CANDIDATES-COUNT  PIC 99.
          05 WS-CANDIDATES PIC X(20) OCCURS 1 TO 20
                DEPENDING ON WS-CANDIDATES-COUNT
                INDEXED BY WS-CANDIDATES-IDX.
      ...
      *    Bubble Sort LHS
           ...
           MOVE 1 TO PTR
           PERFORM VARYING WS-CANDIDATES-IDX FROM 1 BY 1 
               UNTIL WS-CANDIDATES-IDX > WS-CANDIDATES-COUNT
                   IF FUNCTION LOWER-CASE(
                      WS-CANDIDATES(WS-CANDIDATES-IDX))
                      NOT = FUNCTION LOWER-CASE(WS-SUBJECT)

      *               Bubble Sort RHS
                      MOVE FUNCTION LOWER-CASE(
                         WS-CANDIDATES(WS-CANDIDATES-IDX)) TO RHS
      ...
      *               If sorted chars match, they're anagrams
                      IF LHS = RHS
                         IF PTR > 1
                            ...
                            END-STRING
                         END-IF
                         STRING WS-CANDIDATES(WS-CANDIDATES-IDX)
                            DELIMITED BY SPACE
                            INTO WS-RESULT-LIST
                            WITH POINTER PTR
                         END-STRING
